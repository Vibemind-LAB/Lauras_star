...
def test_fetch_torrent_fan_out_idempotent_across_retries(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    src = tmp_path / "src"
    src.mkdir()
    ...
    monkeypatch.setattr(
        h, "aria2_download",
        lambda url, dest_dir, **kw: [src / "a.mp4", src / "b.mp4"],
    )
    ...
    # Run the fetch handler twice for the same asset (simulating a retry re-run).
    for attempt in range(2):
        enqueue(
            db, queue="ingest.io", kind="ingest.fetch",
            payload={"asset_id": asset["id"], "source_url": "magnet:?xt=urn:btih:deadbeef"},
            idempotency_key=f"fetch:{asset['id']}:{attempt}", max_attempts=1,
        )
        _drain(runner)

    assets = repos.list_assets(db, project["id"], limit=50, offset=0)
    # exactly 2 media assets total — no duplicates from the second run
    assert len(assets) == 2
