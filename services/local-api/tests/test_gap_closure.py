...
def test_search_and_transcript_patch(tmp_path: Path) -> None:
    ...
    try:
        ...
        # search_transcript is restricted to each asset's latest SUCCEEDED run (stale-run
        # segments must not double-count after a re-analysis) — mark this run succeeded so its
        # segment is searchable, matching every other seed helper in this repo.
