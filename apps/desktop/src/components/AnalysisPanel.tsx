...
function ShotStrip({ shots, totalFrames }: { shots: Shot[]; totalFrames: number }): ReactElement {
    ...
    <div className="flex h-8 w-full overflow-hidden rounded-md border border-edge">
      {shots.map((s, i) => {
        ...
        return (
          <div
            key={s.id}
            title={`Shot ${i + 1}: ${s.src_in_frame}–${s.src_out_frame_exclusive}`}
...
export function AnalysisPanel({
  ...
  // Load any existing analysis when the asset changes.
  useEffect(() => {
    let cancelled = false;
    setStatus("idle");
    setShots([]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <ShotStrip shots={shots} totalFrames={asset.duration_frames ?? 0} />
      </div>
      ...
      <div>
        ...
        {segments.length === 0 ? (
          ...
          <ul className="max-h-48 space-y-1 overflow-auto">
            {segments.map((seg) => (
              <li key={seg.id} className="rounded-md bg-panel px-3 py-2 text-sm text-slate-200">
                {seg.speaker_label && (
