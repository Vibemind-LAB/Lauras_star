...
/**
 * "Review transitions" — runs the local VLM transition-smoothness review for a timeline and lists
  ...
  return (
    <div className="border-t border-bezel bg-surface-1 px-3 py-2 text-xs">
      <div className="mb-2 flex items-center justify-between">
        <span className="font-medium text-content-muted">Review transitions</span>
        <button
          type="button"
      ...
      {verdicts.length === 0 && !loading && (
        <p className="text-content-faint">
          No verdict yet. "Check" starts the local model (one-time download on the first run).
        </p>
      )}
      ...
      {hasTransitionFix && (
        <p className="mt-1 text-[10px] text-content-faint">
          The crossfade shows in the export/render, not in the live preview.
        </p>
      )}
    </div>
  );
